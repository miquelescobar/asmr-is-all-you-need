...
class WaveRNN(nn.Module):
    def __init__(self, hidden_size=896, quantisation=256):
        ...
        # The main matmul
        self.R = nn.Linear(16+self.hidden_size , 3 * self.hidden_size, bias=False)
    ...
    def forward(self, embedding, prev_y, prev_hidden, current_coarse) :
        ...
        R_hidden = self.R(prev_hidden_with_emb)
    ...
    def generate(self, seq_len, asmr_types) :
        ...
        with torch.no_grad() :
            ...
            embedding = torch.zeros([16], dtype=torch.float).cuda()
            for i in asmr_types:
                embedding = torch.add(embedding, i)
            ...
            for i in range(seq_len) :
                ...
                R_hidden = self.R(hidden_with_emb)
                R_coarse_u , R_fine_u, \
                R_coarse_r, R_fine_r, \
