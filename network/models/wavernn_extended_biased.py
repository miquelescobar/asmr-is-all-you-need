...
class WaveRNN(nn.Module):
    def __init__(self, num_sounds, hidden_size=896, quantisation=256):
        super(WaveRNN, self).__init__()
        
        self.hidden_size = hidden_size
        self.split_size = hidden_size // 2
        self.embedding_dim = 3 * self.split_size
        ...
        # embeddings
        self.embeddings_I_coarse = nn.Embedding(num_embeddings=num_sounds, embedding_dim=self.embedding_dim)
        self.embeddings_I_fine = nn.Embedding(num_embeddings=num_sounds, embedding_dim=self.embedding_dim)
        
        # display num params
        self.num_params()

        
    def forward(self, sound_idx_embed, prev_y, prev_hidden, current_coarse) :
        
        # Main matmul - the projection is split 3 ways
        #prev_hidden_with_emb = torch.cat([prev_hidden, embedding], dim=1)
        # print(prev_hidden.shape, embedding.shape)
        R_hidden = self.R(prev_hidden)
        R_u, R_r, R_e, = torch.split(R_hidden, self.hidden_size, dim=1)
        
        # Project the prev input
        
        embedding_I_coarse = self.embeddings_I_coarse(sound_idx_embed)
                
        coarse_input_proj = self.I_coarse(prev_y) + embedding_I_coarse
        ...
        embedding_I_fine = self.embeddings_I_fine(sound_idx_embed)
        
        fine_input = torch.cat([prev_y, current_coarse], dim=1)
        fine_input_proj = self.I_fine(fine_input) + embedding_I_fine
    ...
    def generate(self, seq_len, asmr_types) :
        
        with torch.no_grad() :
            
            embedding_I_fine = torch.zeros((1, self.embedding_dim)).cuda()
            embedding_I_coarse = torch.zeros((1, self.embedding_dim)).cuda()
            for i in asmr_types:
                embedding_I_fine += self.embeddings_I_fine.weight[i]
                embedding_I_coarse += self.embeddings_I_coarse.weight[i]
                #embedding += self.embeddings.weight[i]
                
            
            embedding_I_fine = embedding_I_fine/len(asmr_types)
            embedding_I_coarse = embedding_I_coarse/len(asmr_types)
            #embedding = embedding/len(asmr_types)
            ...
            for i in range(seq_len) :
                ...
                # Project input 
                coarse_input_proj = self.I_coarse(prev_outputs) + embedding_I_coarse
                I_coarse_u, I_coarse_r, I_coarse_e = \
                    torch.split(coarse_input_proj, self.split_size, dim=1)

                # Project hidden state and split 6 ways
                #hidden_with_emb = torch.cat([hidden, embedding], dim=1)
    ...
    def init_embedding(self, batch_size=1):
        return torch.ones(batch_size, dtype=torch.long).cuda()
