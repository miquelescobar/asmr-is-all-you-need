...
class ResidualBlock(torch.nn.Module):
    def __init__(self, res_channels, skip_channels, dilation):
        ...
        # We will use the same embedding for both sigmoid and tanh.
        # if self.global_conditioning:
        #     self.global_conditioning_weight_tanh = nn.Linear(bias=False)
        #     self.global_conditioning_weight_sigmoid = nn.Linear(bias=False)

    def forward(self, x, skip_size, class_embedding=None):
        """
        :param x:
        ...
        # PixelCNN gate with global conditioning
        gated_tanh = self.gate_tanh(output)
        gated_sigmoid = self.gate_sigmoid(output)
        # print(gated_tanh.shape, gated_sigmoid.shape)
        # print('-----')
        if class_embedding:
            gated_tanh += class_embedding
            gated_sigmoid += class_embedding
        # if self.global_conditioning:
        #     gated_tanh += self.global_conditioning_weight_tanh(class_embedding)
        #     gated_sigmoid += self.global_conditioning_weight_tanh(class_embedding)
...
class ResidualStack(torch.nn.Module):
    ...
    def forward(self, x, skip_size, class_embedding):
        """
        :param x:
        :param skip_size: The last output size for loss and prediction
        :return:
        ...
        for res_block in self.res_blocks:
            ...
            output, skip = res_block(output, skip_size, class_embedding)
...
class WaveNet(torch.nn.Module):
    def __init__(self, layer_size, stack_size, in_channels, res_channels, num_global_classes):
        """
        Stack residual blocks by layer and stack size
        :param layer_size: integer, 10 = layer[dilation=1, dilation=2, 4, 8, 16, 32, 64, 128, 256, 512]
        :param stack_size: integer, 5 = stack[layer1, layer2, layer3, layer4, layer5]
        ...
        if self.num_global_classes:
            self.global_embedding = nn.Embedding(num_global_classes, res_channels)
    ...
    def forward(self, x, class_num=0):
        ...
        if self.num_global_classes:
            class_embedding = self.global_embedding(class_num)
        else:
