...
def one_hot_decode(data, axis=1):
    ...
    decoded = np.zeros((data.shape[0], data.shape[1], 1))
    for i in range(data.shape[1]):
        probs = data[0,i] / np.sum(data[0,i])
        decoded[0, i] = np.random.choice(np.arange(0,data.shape[2]), p=probs)
    ...
    def _collate_fn(self, audio):
        ...
        if self.sample_size:
            ...
            while sample_size > self.receptive_fields:
                ...
                x_random = np.random.randint(3000)
                audio = audio[:, sample_size-self.receptive_fields-x_random:, :]
                sample_size = self.calc_sample_size(audio)
